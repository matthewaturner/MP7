...
struct ET_PARAMS {
/* Event Handler Thread Parameters */
	BoundedBuffer<Request> *request_buffer; // where to get the requests
	RequestChannel *chan;                   // control channel to database
	int tot_num_requests;                   // total num requests
	int num_channels;                       // num worker channels to create
};
...
void* et_func(void* arg) {

	// handle parameters
	ET_PARAMS p = *(ET_PARAMS *)arg;
	BoundedBuffer<Request> *request_buffer = p.request_buffer;
	RequestChannel *chan                   = p.chan;
	int tot_num_requests                   = p.tot_num_requests;
	int num_channels                       = p.num_channels;

	// set up request channels
	RequestChannel *channels[num_channels];
	for(int i=0; i<num_channels; i++) {
		std::string s = chan->send_request("newthread");
		channels[i] = new RequestChannel(s, RequestChannel::CLIENT_SIDE);
	}

	// where to put each channel's responses
	BoundedBuffer<std::string> *response_buffers[num_channels];
	...
	// first fill all of the channels with requests
	for(int i=0; i<num_channels; i++) {
		// take an item off the buffer
		Request r = request_buffer->pop();

		// send message to dataserver over channel i
		std::string msg = "data " + r.name;
		channels[i]->cwrite(msg);

		// remember where this channel's response will go
		response_buffers[i] = r.response_buffer;

		// update progress
		channels_written++;
	}

	while(channels_read < tot_num_requests) {
		...
		select_response = select(max+1, &rfds, NULL, NULL, &tv);
		
		if(select_response) {
			for(int i=0; i<num_channels; i++) {
				// if there is a response in the channel
				if(FD_ISSET(channels[i]->read_fd(), &rfds)) {
					// ----- READ FROM CHANNEL -----

					// read response
					std::string response = channels[i]->cread();
					response_buffers[i]->push(response);

					// update progress
					channels_read++;

					if(channels_written < tot_num_requests) {
						// ----- REUSE CHANNEL -----

						// get a new response from the buffer
						Request r = request_buffer->pop();

						// send message to dataserver over channel
						std::string msg = "data " + r.name;
						channels[i]->cwrite(msg);
					...
					//aso.print("...");
					//aso.print(to_string(channels_read));
					//aso.print(to_string(channels_written));
				}
			}
		}

		if(channels_read == tot_num_requests)
			break;
	}
...
int main(int argc, char * argv[]) {
	...
	// parameters for event thread
	ET_PARAMS et_params = {&request_buffer, chan, n*3, w};

	// create event handler thread
	pthread_create(&et_id, 0, &et_func, (void *)&et_params);
